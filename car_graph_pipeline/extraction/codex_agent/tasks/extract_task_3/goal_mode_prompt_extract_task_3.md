...
目标：完成 `/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3` 中汽车说明书的 GraphRAG 抽取准备、chunk 抽取、chunk review/self-review、doc raw 合并、程序化校验和结果汇总。

你只负责当前 task 目录：
`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3`

不要读取、修改或写入其他 `extract_task_*` 目录。

## 关键路径

- 原始文档目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_doc`
- 本任务代码目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_code`
- 本任务输出目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_output`
- 本任务 manifest：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_output/task_manifest.json`
- 车型映射文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_output/vehicle_scope_map.json`
- 车型映射待审文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_output/scope_review_needed.json`
- 本任务 few-shot 示例：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
1. `task_3_output/task_manifest.json`
2. `task_3_code/README_CODE.md`
3. `/Users/lzj/proj/car_graph/car_graph_pipeline/extract/chunk_gate_extraction_sop.md`
4. `task_3_output/README_OUTPUT_LAYOUT.md`
5. `task_3_output/examples/car_manual_graph_extract_few_shots.v1.json`

确认当前 task 有 204 个文档后，直接开始执行，不要停留在方案阶段。

## 强约束

- 所有代码改造都放在 `task_3_code`，不要直接修改原始 `car_graph_pipeline` 代码。
- 所有结果都放在 `task_3_output`，不要写入其他 task 输出目录。
- 不要写 HugeGraph，不要写当前 `car_graph` 图。
- 不要覆盖 v3 结果目录 `output/new_doc_agent_v3_semantic`。
- 每个文档独立使用 `task_3_output/doc_runs/<doc_run_id>/`。
...
- `task_3_output/semantic_chunks`
- `task_3_output/risk_classification`
- `task_3_output/examples`
- `task_3_output/doc_runs`
- `task_3_output/doc_raw_results`
- `task_3_output/validated_results`
- `task_3_output/analysis`
- `task_3_output/blocked`
- `task_3_output/checkpoints`
- `task_3_output/merged`
- `task_3_output/locks`
- `task_3_output/prompts`
- `task_3_output/run_logs`

每个文档目录：

- `task_3_output/doc_runs/<doc_run_id>/source_info.json`
- `task_3_output/doc_runs/<doc_run_id>/chunks`
- `task_3_output/doc_runs/<doc_run_id>/chunk_drafts`
- `task_3_output/doc_runs/<doc_run_id>/chunk_reviews`
- `task_3_output/doc_runs/<doc_run_id>/chunk_self_reviews`
- `task_3_output/doc_runs/<doc_run_id>/chunk_final`
- `task_3_output/doc_runs/<doc_run_id>/doc_raw`
- `task_3_output/doc_runs/<doc_run_id>/state`
- `task_3_output/doc_runs/<doc_run_id>/logs`
- `task_3_output/doc_runs/<doc_run_id>/intents`
- `task_3_output/doc_runs/<doc_run_id>/scratch`

不要自行发明新的主目录。确实需要新目录时，放在当前 doc_run 的 `scratch/` 或 task 级 `analysis/` 下。

## 已提供的 task 内固定代码

`task_3_code` 中已经提供以下固定脚本。优先运行/小修这些脚本，不要从零重写，也不要改原始 `car_graph_pipeline` 代码：
...
```bash
cd /Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_3/task_3_code
...
`task_3_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
`task_3_output/vehicle_scope_map.json`
...
2. 为每个 doc-run 创建 lock：
   - `task_3_output/locks/<doc_run_id>.lock`
...
兜底规则：extractor 不能无限等待 reviewer。`poll_seconds` 不应低于一次正常 reviewer 响应时间，当前建议 180 秒；如果连续 3 次合理轮询后 `state.json` 仍没有从 `awaiting_review` 变为 `passed` / `needs_fix` / `blocked`，extractor 必须写 `chunk_self_reviews/<chunk_id>.attempt<N>.review.json`，标注 `reviewer_timeout_fallback=true` 和等待次数。自审通过则写 `chunk_final/<chunk_id>.final.json` 并推进；自审失败则只修当前 chunk 并重提，达到 `max_attempts` 后写 blocked。该兜底只解决 reviewer 超时，不允许跳过 draft、evidence、schema、端点或关系方向检查。

状态一致性规则：每次写 `chunk_final` 后，extractor 必须同步更新 `completed_chunks`、`current_index`、`current_chunk`、`phase`、`attempt`、`draft_path`、`review_path`、`final_path`。如果发现 `chunk_final` 文件数量、`completed_chunks`、`current_index/current_chunk` 不一致，extractor 应以 `chunk_final/*.final.json` 和 `chunk_ids` 顺序为准自修复 `state.json`，并在 `logs/` 或 `state.events` 记录 reconciliation；reviewer 发现 state 已被推进或 final 已存在时不得覆盖旧 chunk 的状态。
