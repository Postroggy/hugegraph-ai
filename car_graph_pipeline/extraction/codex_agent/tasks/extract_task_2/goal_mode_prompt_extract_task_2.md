...
目标：完成 `/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2` 中汽车说明书的 GraphRAG 抽取准备、chunk 抽取、chunk review/self-review、doc raw 合并、程序化校验和结果汇总。

你只负责当前 task 目录：
`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2`

不要读取、修改或写入其他 `extract_task_*` 目录。

## 关键路径

- 原始文档目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_doc`
- 本任务代码目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_code`
- 本任务输出目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_output`
- 本任务 manifest：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_output/task_manifest.json`
- 车型映射文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_output/vehicle_scope_map.json`
- 车型映射待审文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_output/scope_review_needed.json`
- 本任务 few-shot 示例：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
1. `task_2_output/task_manifest.json`
2. `task_2_code/README_CODE.md`
3. `/Users/lzj/proj/car_graph/car_graph_pipeline/extract/chunk_gate_extraction_sop.md`
4. `task_2_output/README_OUTPUT_LAYOUT.md`
5. `task_2_output/examples/car_manual_graph_extract_few_shots.v1.json`

确认当前 task 有 204 个文档后，直接开始执行，不要停留在方案阶段。

## 强约束

- 所有代码改造都放在 `task_2_code`，不要直接修改原始 `car_graph_pipeline` 代码。
- 所有结果都放在 `task_2_output`，不要写入其他 task 输出目录。
- 不要写 HugeGraph，不要写当前 `car_graph` 图。
- 不要覆盖 v3 结果目录 `output/new_doc_agent_v3_semantic`。
- 每个文档独立使用 `task_2_output/doc_runs/<doc_run_id>/`。
...
- `task_2_output/semantic_chunks`
- `task_2_output/risk_classification`
- `task_2_output/examples`
- `task_2_output/doc_runs`
- `task_2_output/doc_raw_results`
- `task_2_output/validated_results`
- `task_2_output/analysis`
- `task_2_output/blocked`
- `task_2_output/checkpoints`
- `task_2_output/merged`
- `task_2_output/locks`
- `task_2_output/prompts`
- `task_2_output/run_logs`

每个文档目录：

- `task_2_output/doc_runs/<doc_run_id>/source_info.json`
- `task_2_output/doc_runs/<doc_run_id>/chunks`
- `task_2_output/doc_runs/<doc_run_id>/chunk_drafts`
- `task_2_output/doc_runs/<doc_run_id>/chunk_reviews`
- `task_2_output/doc_runs/<doc_run_id>/chunk_self_reviews`
- `task_2_output/doc_runs/<doc_run_id>/chunk_final`
- `task_2_output/doc_runs/<doc_run_id>/doc_raw`
- `task_2_output/doc_runs/<doc_run_id>/state`
- `task_2_output/doc_runs/<doc_run_id>/logs`
- `task_2_output/doc_runs/<doc_run_id>/intents`
- `task_2_output/doc_runs/<doc_run_id>/scratch`

不要自行发明新的主目录。确实需要新目录时，放在当前 doc_run 的 `scratch/` 或 task 级 `analysis/` 下。

## 已提供的 task 内固定代码

`task_2_code` 中已经提供以下固定脚本。优先运行/小修这些脚本，不要从零重写，也不要改原始 `car_graph_pipeline` 代码：
...
```bash
cd /Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_2/task_2_code
...
`task_2_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
`task_2_output/vehicle_scope_map.json`
...
2. 为每个 doc-run 创建 lock：
   - `task_2_output/locks/<doc_run_id>.lock`
...
- extractor 不能无限等待 reviewer。`poll_seconds` 必须不低于一次正常 reviewer 响应时间，当前默认 180 秒；连续 `review_timeout_polls=3` 次仍无 reviewer 结果时，extractor 写结构化 timeout self-review，并标注 `reviewer_timeout_fallback=true`。
- timeout self-review 通过时，extractor 可把当前最新 draft 写入 `chunk_final/<chunk_id>.final.json` 并推进下一个 chunk；timeout self-review 不通过时，只修当前 chunk 并再次提交，达到 `max_attempts` 后写 blocked。
...
也就是说，高风险/中风险 chunk 是同步 gate：同一个 extractor 的 chunk 序列必须停在当前 chunk，直到 reviewer pass/fail/block 处理完毕；但 reviewer 长时间无响应时，允许按 timeout self-review 兜底，不能让抽取无限卡住。

如果当前环境不方便长时间维护双 agent 自动状态机，可以由主会话按 SOP 调度，但仍必须按文件落盘。

主控会话不要频繁插手内部抽取/review。主控只做低频健康检查，约每 15 分钟查看一次 `state.json` 的 phase、current_index、completed/blocked chunk 数、updated_at 和文件计数；只有 blocked、长时间无进展、JSON 损坏或状态冲突时才介入。
