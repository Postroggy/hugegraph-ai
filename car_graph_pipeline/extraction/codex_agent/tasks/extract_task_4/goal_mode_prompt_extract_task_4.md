...
目标：完成 `/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4` 中汽车说明书的 GraphRAG 抽取准备、chunk 抽取、chunk review/self-review、doc raw 合并、程序化校验和结果汇总。

你只负责当前 task 目录：
`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4`

不要读取、修改或写入其他 `extract_task_*` 目录。

## 关键路径

- 原始文档目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_doc`
- 本任务代码目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_code`
- 本任务输出目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_output`
- 本任务 manifest：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_output/task_manifest.json`
- 车型映射文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_output/vehicle_scope_map.json`
- 车型映射待审文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_output/scope_review_needed.json`
- 本任务 few-shot 示例：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
1. `task_4_output/task_manifest.json`
2. `task_4_code/README_CODE.md`
3. `/Users/lzj/proj/car_graph/car_graph_pipeline/extract/chunk_gate_extraction_sop.md`
4. `task_4_output/README_OUTPUT_LAYOUT.md`
5. `task_4_output/examples/car_manual_graph_extract_few_shots.v1.json`

确认当前 task 有 204 个文档后，直接开始执行，不要停留在方案阶段。

## 强约束

- 所有代码改造都放在 `task_4_code`，不要直接修改原始 `car_graph_pipeline` 代码。
- 所有结果都放在 `task_4_output`，不要写入其他 task 输出目录。
- 不要写 HugeGraph，不要写当前 `car_graph` 图。
- 不要覆盖 v3 结果目录 `output/new_doc_agent_v3_semantic`。
- 每个文档独立使用 `task_4_output/doc_runs/<doc_run_id>/`。
...
- `task_4_output/semantic_chunks`
- `task_4_output/risk_classification`
- `task_4_output/examples`
- `task_4_output/doc_runs`
- `task_4_output/doc_raw_results`
- `task_4_output/validated_results`
- `task_4_output/analysis`
- `task_4_output/blocked`
- `task_4_output/checkpoints`
- `task_4_output/merged`
- `task_4_output/locks`
- `task_4_output/prompts`
- `task_4_output/run_logs`

每个文档目录：

- `task_4_output/doc_runs/<doc_run_id>/source_info.json`
- `task_4_output/doc_runs/<doc_run_id>/chunks`
- `task_4_output/doc_runs/<doc_run_id>/chunk_drafts`
- `task_4_output/doc_runs/<doc_run_id>/chunk_reviews`
- `task_4_output/doc_runs/<doc_run_id>/chunk_self_reviews`
- `task_4_output/doc_runs/<doc_run_id>/chunk_final`
- `task_4_output/doc_runs/<doc_run_id>/doc_raw`
- `task_4_output/doc_runs/<doc_run_id>/state`
- `task_4_output/doc_runs/<doc_run_id>/logs`
- `task_4_output/doc_runs/<doc_run_id>/intents`
- `task_4_output/doc_runs/<doc_run_id>/scratch`

不要自行发明新的主目录。确实需要新目录时，放在当前 doc_run 的 `scratch/` 或 task 级 `analysis/` 下。

## 已提供的 task 内固定代码

`task_4_code` 中已经提供以下固定脚本。优先运行/小修这些脚本，不要从零重写，也不要改原始 `car_graph_pipeline` 代码：
...
```bash
cd /Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_4/task_4_code
...
`task_4_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
`task_4_output/vehicle_scope_map.json`
...
2. 为每个 doc-run 创建 lock：
   - `task_4_output/locks/<doc_run_id>.lock`
...
- max attempts 后写 chunk 级 blocked，记录原因，然后 extractor 推进后续 chunk/doc；不要因为单个 chunk blocked 就把整个 doc 停死

reviewer 无响应兜底：

- `poll_seconds` 不低于 180 秒，避免短于正常一次 review 响应时间。
- extractor 对 `phase=awaiting_review` 连续轮询 3 次仍没有新的 `review_path` 或 phase 变化时，允许启动 extractor self-review 兜底。
- 兜底时必须写 `chunk_self_reviews/<chunk_id>.attempt<N>.review.json`，并在 review 中标记 `fallback_reason=reviewer_timeout`。
- self-review pass 后写 `chunk_final/<chunk_id>.final.json` 并推进；self-review fail 且可修复时只修当前 chunk；达到 max attempts 后写 chunk 级 blocked 并推进。
- fallback 只能用于 reviewer 无响应，不得用于绕过 reviewer 已经给出的 fail/block。

也就是说，高风险/中风险 chunk 是同步 gate：同一个 extractor 的 chunk 序列必须停在当前 chunk，直到 reviewer pass/fail/block 处理完毕。

如果当前环境不方便长时间维护双 agent 自动状态机，可以由主会话按 SOP 调度，但仍必须按文件落盘。主会话只做粗粒度健康检查，默认每 15 分钟检查一次 state、final/review/blocked 计数和最近文件更新时间，不读取具体 chunk/draft/review 内容，不逐 chunk 干预内部抽取。
