...
BRAND_HINTS = {
    "斯威": ["斯威", "SWM"],
    "斯巴鲁": ["斯巴鲁", "Subaru", "旭豹", "森林人", "欧霸"],
    "吉利": ["吉利", "博越", "缤瑞", "星瑞", "星越", "缤越", "熊猫", "枫叶"],
    "奇瑞": ["奇瑞", "瑞虎", "星途"],
    "宝骏": ["宝骏", "新宝骏", "RC-6", "RS-3", "悦也", "缤果"],
    "金杯": ["金杯", "新海狮", "新豹", "阁瑞斯"],
    "斯柯达": ["斯柯达", "昊锐", "明锐", "昕锐", "晶锐", "柯珞克", "柯米克", "柯迪亚克"],
    "雪佛兰": ["雪佛兰", "景程", "沃兰多", "畅巡", "科帕奇", "科沃兹", "科迈罗", "科鲁兹"],
    "智己": ["智己", "L6", "L7", "LS6", "LS7", "LS8", "LS9"],
    "标致": ["标致"],
    "欧拉": ["欧拉", "黑猫"],
    "宾利": ["宾利", "欧陆"],
    "沃尔沃": ["沃尔沃"],
    ...
    "Jeep": ["Jeep", "牧马人"],
    "起亚": ["起亚", "狮铂拓界", "索兰托"],
    "现代": ["现代", "IONIQ", "索纳塔", "朗动", "瑞纳"],
    "理想": ["理想"],
    "红旗": ["红旗"],
    "五十铃": ["五十铃", "翼放"],
    "捷豹路虎": ["路虎", "神行者"],
    "广汽传祺": ["传祺", "祺智"],
    "福特": ["福特", "EVOS", "F-150", "Mustang", "探险者", "烈马", "电马", "福睿斯", "福克斯", "蒙迪欧", "锐界", "锐际", "领界", "领裕", "翼搏", "翼虎"],
    "丰田": ["丰田", "Toyota", "Aygo", "C-HR", "RAV4", "YARiS", "HIACE", "MIRAI", "SUPRA", "普拉多", "普锐斯", "普瑞维亚", "埃尔法", "亚洲狮", "汉兰达", "凯美瑞", "皇冠", "威尔法", "考斯特"],
    "别克": ["别克", "GL8", "世纪", "君威", "凯越", "昂科威", "昂科拉", "昂科旗", "昂科雷", "林荫大道"],
    ...
    "本田": ["本田", "Honda", "CR-V", "CIIMO", "LIFE", "VE-1", "HR-V", "XR-V", "杰德", "歌诗图", "皓影", "缤智", "极湃"],
    ...
    "哈弗": ["哈弗", "H2", "H4", "H5", "H6", "H7", "Dagou", "大狗", "神兽", "枭龙"],
    "比亚迪": ["比亚迪", "BYD", "F0", "F3", "F3R", "F6", "G3", "G3R", "G5", "G6", "M6", "S6", "e2", "e6", "宋MAX", "汉", "海狮", "海豚", "海豹", "海鸥", "秦"],
    "大众": ["大众", "Volkswagen", "CC", "ID.4", "ID.6", "Magotan", "Polo", "T-ROC", "迈腾", "朗逸", "朗行", "朗境", "桑塔纳", "甲壳虫"],
...
def infer_scope(doc_name: str) -> dict[str, str]:
    stem = re.sub(r"\.md$", "", doc_name)
    brand = ""
    for candidate, hints in BRAND_HINTS.items():
        if any(hint and hint in stem for hint in hints):
            brand = candidate
            break
    model = stem
    model = re.sub(r"(用户手册|说明书|使用说明书|Owner.s Manual|Owner's Manual|pdf)$", "", model, flags=re.IGNORECASE)
    model = re.sub(r"^\d{4}款", "", model).strip(" -_")
    confidence = "high" if brand else "low"
    return {
        "vehicle_brand": brand or "UNKNOWN_BRAND",
        "vehicle_model": model or stem,
...
def ensure_scope_map() -> dict[str, Any]:
    ...
    for item in manifest:
        ...
        if doc_name not in existing:
            ...
            inferred = infer_scope(doc_name)
            current = existing[doc_name]
            if current.get("vehicle_brand") == "UNKNOWN_BRAND" and inferred.get("confidence") == "high":
                existing[doc_name] = inferred
                changed = True
                review_by_doc.pop(doc_name, None)
    ...
    for item in selected:
        ...
        state = {
            ...
            **scope,
            ...
            "max_attempts": 3,
            "poll_seconds": 180,
            "reviewer_timeout_polls": 3,
            "reviewer_timeout_fallback": "extractor_self_review",
            "completed_chunks": [],
            "blocked_chunks": [],
            "events": [],
...
def summarize_progress(_args: argparse.Namespace) -> None:
    ...
    for item in manifest:
        ...
        chunk_blocked = list((out_dir / "blocked").glob(f"{item['doc_run_id']}*.blocked.json"))
        state_path = doc_run_dir / "state" / "state.json"
        state = json.loads(state_path.read_text(encoding="utf-8")) if state_path.exists() else {}
        doc_blocked = state.get("phase") == "blocked" and not state.get("current_chunk")
        validated = out_dir / "validated_results" / f"{item['doc_run_id']}.validated.json"
        total_chunks += len(chunks)
        completed_chunks += len(finals)
        blocked_chunks += len(chunk_blocked)
        if validated.exists():
            completed += 1
        if doc_blocked:
            blocked_docs += 1
