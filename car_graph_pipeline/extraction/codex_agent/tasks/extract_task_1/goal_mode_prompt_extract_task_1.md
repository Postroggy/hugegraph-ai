...
目标：完成 `/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1` 中汽车说明书的 GraphRAG 抽取准备、chunk 抽取、chunk review/self-review、doc raw 合并、程序化校验和结果汇总。
...
- 本任务代码目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1/task_1_code`
- 本任务输出目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1/task_1_output`
- 本任务 manifest：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1/task_1_output/task_manifest.json`
- 车型映射文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1/task_1_output/vehicle_scope_map.json`
- 车型映射待审文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1/task_1_output/scope_review_needed.json`
- 本任务 few-shot 示例：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_1/task_1_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
1. `task_1_output/task_manifest.json`
2. `task_1_code/README_CODE.md`
3. `/Users/lzj/proj/car_graph/car_graph_pipeline/extract/chunk_gate_extraction_sop.md`
4. `task_1_output/README_OUTPUT_LAYOUT.md`
5. `task_1_output/examples/car_manual_graph_extract_few_shots.v1.json`

确认当前 task 有 204 个文档后，直接开始执行，不要停留在方案阶段。

## 强约束

- 所有代码改造都放在 `task_1_code`，不要直接修改原始 `car_graph_pipeline` 代码。
- 所有结果都放在 `task_1_output`，不要写入其他 task 输出目录。
- 不要写 HugeGraph，不要写当前 `car_graph` 图。
- 不要覆盖 v3 结果目录 `output/new_doc_agent_v3_semantic`。
- 每个文档独立使用 `task_1_output/doc_runs/<doc_run_id>/`。
...
- `task_1_output/semantic_chunks`
- `task_1_output/risk_classification`
- `task_1_output/examples`
- `task_1_output/doc_runs`
- `task_1_output/doc_raw_results`
- `task_1_output/validated_results`
- `task_1_output/analysis`
- `task_1_output/blocked`
- `task_1_output/checkpoints`
- `task_1_output/merged`
- `task_1_output/locks`
- `task_1_output/prompts`
- `task_1_output/run_logs`

每个文档目录：

- `task_1_output/doc_runs/<doc_run_id>/source_info.json`
- `task_1_output/doc_runs/<doc_run_id>/chunks`
- `task_1_output/doc_runs/<doc_run_id>/chunk_drafts`
- `task_1_output/doc_runs/<doc_run_id>/chunk_reviews`
- `task_1_output/doc_runs/<doc_run_id>/chunk_self_reviews`
- `task_1_output/doc_runs/<doc_run_id>/chunk_final`
- `task_1_output/doc_runs/<doc_run_id>/doc_raw`
- `task_1_output/doc_runs/<doc_run_id>/state`
- `task_1_output/doc_runs/<doc_run_id>/logs`
- `task_1_output/doc_runs/<doc_run_id>/intents`
- `task_1_output/doc_runs/<doc_run_id>/scratch`

不要自行发明新的主目录。确实需要新目录时，放在当前 doc_run 的 `scratch/` 或 task 级 `analysis/` 下。

## 已提供的 task 内固定代码

`task_1_code` 中已经提供以下固定脚本。优先运行/小修这些脚本，不要从零重写，也不要改原始 `car_graph_pipeline` 代码：
...
`task_1_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
`task_1_output/vehicle_scope_map.json`
...
2. 为每个 doc-run 创建 lock：
   - `task_1_output/locks/<doc_run_id>.lock`
...
主会话只做外部调度和健康检查，不做内容级抽取/review：

- 默认每 15 分钟检查一次活跃 doc-run 的 `state.json`、phase、attempt、chunk_final 数量和坏 JSON。
- 主会话不要读取大段 doc/chunk/draft 内容替内部 reviewer 判断质量。
- 只有协议级问题才介入，例如长时间无文件变化、坏 JSON、路径写错、state 冲突、agent 退出、无法 merge/validate。
...
- extractor 提交 `awaiting_review` 后，按 `poll_seconds` 等待 reviewer。
- `poll_seconds` 默认不低于一次正常 reviewer 响应时间，当前建议 180 秒。
- 如果连续 3 次轮询仍没有 reviewer 结果，extractor 不再无限等待。
- extractor 写 `chunk_self_reviews/<chunk_id>.attempt<N>.timeout_self_review.json`，标注 `reviewer_timeout_fallback=true`。
- 自审通过则写 `chunk_final/<chunk_id>.final.json` 并推进下一个 chunk；自审不通过则按当前 chunk 自行修正并重试，达到 max attempts 后写 blocked。
- timeout fallback 是兜底机制，不改变“优先 reviewer gate”的原则。
