# Goal Mode Prompt Template - Final

把下面 prompt 中的 `<TASK_NUM>` 替换为当前会话负责的任务编号，例如 `1`、`2`、`3`、`4`、`5`。

```text
你现在进入 goal 模式。

目标：完成 `/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>` 中汽车说明书的 GraphRAG 抽取准备、chunk 抽取、chunk review/self-review、doc raw 合并、程序化校验和结果汇总。

你只负责当前 task 目录：
`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>`

不要读取、修改或写入其他 `extract_task_*` 目录。

## 关键路径

- 原始文档目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_doc`
- 本任务代码目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_code`
- 本任务输出目录：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_output`
- 本任务 manifest：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_output/task_manifest.json`
- 车型映射文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_output/vehicle_scope_map.json`
- 车型映射待审文件：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_output/scope_review_needed.json`
- 本任务 few-shot 示例：`/Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
1. `task_<TASK_NUM>_output/task_manifest.json`
2. `task_<TASK_NUM>_code/README_CODE.md`
3. `/Users/lzj/proj/car_graph/car_graph_pipeline/extract/chunk_gate_extraction_sop.md`
4. `task_<TASK_NUM>_output/README_OUTPUT_LAYOUT.md`
5. `task_<TASK_NUM>_output/examples/car_manual_graph_extract_few_shots.v1.json`

确认当前 task 有 204 个文档后，直接开始执行，不要停留在方案阶段。

## 强约束

- 所有代码改造都放在 `task_<TASK_NUM>_code`，不要直接修改原始 `car_graph_pipeline` 代码。
- 所有结果都放在 `task_<TASK_NUM>_output`，不要写入其他 task 输出目录。
- 不要写 HugeGraph，不要写当前 `car_graph` 图。
- 不要覆盖 v3 结果目录 `output/new_doc_agent_v3_semantic`。
- 每个文档独立使用 `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/`。
...
- `task_<TASK_NUM>_output/semantic_chunks`
- `task_<TASK_NUM>_output/risk_classification`
- `task_<TASK_NUM>_output/examples`
- `task_<TASK_NUM>_output/doc_runs`
- `task_<TASK_NUM>_output/doc_raw_results`
- `task_<TASK_NUM>_output/validated_results`
- `task_<TASK_NUM>_output/analysis`
- `task_<TASK_NUM>_output/blocked`
- `task_<TASK_NUM>_output/checkpoints`
- `task_<TASK_NUM>_output/merged`
- `task_<TASK_NUM>_output/locks`
- `task_<TASK_NUM>_output/prompts`
- `task_<TASK_NUM>_output/run_logs`

每个文档目录：

- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/source_info.json`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/chunks`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/chunk_drafts`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/chunk_reviews`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/chunk_self_reviews`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/chunk_final`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/doc_raw`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/state`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/logs`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/intents`
- `task_<TASK_NUM>_output/doc_runs/<doc_run_id>/scratch`

不要自行发明新的主目录。确实需要新目录时，放在当前 doc_run 的 `scratch/` 或 task 级 `analysis/` 下。

## 已提供的 task 内固定代码

`task_<TASK_NUM>_code` 中已经提供以下固定脚本。优先运行/小修这些脚本，不要从零重写，也不要改原始 `car_graph_pipeline` 代码：
...
2. `prepare_chunks.py`
   ...
   - 输入当前 task 的 `task_<TASK_NUM>_doc`。
...
```bash
cd /Users/lzj/proj/car_graph/all_doc/docs_tasks/extract_task_<TASK_NUM>/task_<TASK_NUM>_code
...
`task_<TASK_NUM>_output/examples/car_manual_graph_extract_few_shots.v1.json`
...
`task_<TASK_NUM>_output/vehicle_scope_map.json`
...
2. 为每个 doc-run 创建 lock：
   - `task_<TASK_NUM>_output/locks/<doc_run_id>.lock`
