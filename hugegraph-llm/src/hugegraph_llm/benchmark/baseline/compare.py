...
class ComparisonResult(BaseModel):
    ...
    # Both sides' run metadata (timestamp, git_commit, model, data_path,
    # data_meta, …). A compare-mode report is unreadable without this — the
    # reviewer needs to know "was it the same data / same commit / same model"
    # to diagnose where the delta came from. Kept as-is from
    # ``BenchmarkResult.metadata`` so no field is dropped.
...
class BaselineComparator:
    ...
    def compare(
        ...
        # Preserve both sides' run metadata so the compare report can show a
        # side-by-side environment block. Without this a reviewer can't tell
        # if the delta is caused by the change under test, a data swap, or a
        # different judge model.
        result.baseline_metadata = dict(baseline.metadata)
        result.candidate_metadata = dict(candidate.metadata)

        # Overall diff is direction-aware: positive means improvement.
        # ``*_redundancy`` are diagnostic scores (lower-is-better, not quality
        # metrics) — exclude them so they neither drive 退化/改进 verdicts nor
        # get flagged with an inverted direction.
        all_keys = {
            k for k in (set(baseline.overall.keys()) | set(candidate.overall.keys()))
            if not k.endswith("_redundancy")
        }
