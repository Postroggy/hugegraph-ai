...
def normalize_graph_extract(
    ...
    Example:
        ...
        >>> normalize_graph_extract(data)
        {
            ...
            "candidate_edges": [
                {"label": "knows", "outV": "Alice", "outVLabel": "person",
                 "inV": "Bob", "inVLabel": "person", "properties": {}},
            ],
        }
    """
    ...
    for edge in edges:
        ...
        edge_record: Dict[str, Any] = {"label": label}
        edge_record["outV"] = _resolve_endpoint(raw_out, id_to_name)
        out_v_label = edge.get("outVLabel")
        if out_v_label:
            edge_record["outVLabel"] = out_v_label
        edge_record["inV"] = _resolve_endpoint(raw_in, id_to_name)
        in_v_label = edge.get("inVLabel")
        if in_v_label:
            edge_record["inVLabel"] = in_v_label
        edge_record["properties"] = properties if isinstance(properties, dict) else {}
        candidate_edges.append(edge_record)
