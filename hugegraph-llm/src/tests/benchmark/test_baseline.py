...
def test_baselinecomparator_excludes_redundancy_from_regression():
    """Redundancy is a diagnostic, lower-is-better score; it must NOT enter
    overall_diff / metric_verdicts — otherwise its inverted direction would
    flag the wrong side as 退化/改进 and a diagnostic would drive verdicts."""
    baseline = _make_result([{'semantic_entity_f1': 0.8, 'semantic_entity_redundancy': 0.2}])
    candidate = _make_result([{'semantic_entity_f1': 0.9, 'semantic_entity_redundancy': 0.5}])
    comparison = BaselineComparator.compare(baseline, candidate)
    assert 'semantic_entity_redundancy' not in comparison.overall_diff
    assert 'semantic_entity_redundancy' not in comparison.analyze()['metric_verdicts']
    # The quality metric is still compared normally.
    assert 'semantic_entity_f1' in comparison.overall_diff
    assert comparison.overall_diff['semantic_entity_f1'] > 0


def test_baselinecomparator_excludes_triple_redundancy_from_regression():
    baseline = _make_result([{'semantic_triple_redundancy': 0.1}])
    candidate = _make_result([{'semantic_triple_redundancy': 0.4}])
    comparison = BaselineComparator.compare(baseline, candidate)
    assert 'semantic_triple_redundancy' not in comparison.overall_diff
